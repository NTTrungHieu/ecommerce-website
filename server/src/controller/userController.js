...
const updatePassword = asyncHandler(async (req, res) => {
  ...
  if (isPasswordMatched) {
    ...
    throw new Error("Your Current Password is different.");
  }
  res.json(user);
});
...
const forgotPasswordToken = asyncHandler(async (req, res) => {
  ...
  <br>Please follow this link to reset your password. This link is valid till 10 minutes from now. <a href="${process.env.API_URL}/user/reset-password/${token}/${randomPassword}">Click Here</a>`;
  const data = {
    to: email,
...
const resetPassword = asyncHandler(async (req, res) => {
  ...
  await user.save();
  res.redirect(process.env.CLIENT_URL+"/login?reset=success");
});
